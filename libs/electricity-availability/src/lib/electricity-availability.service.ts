  ...
  try {
    ...
    if (!latest.is_available) {
      ...
      if (lastScheduled.status === LightStatus.ON) {
        // Сценарій 1a: Вимкнули під час планового СВІТЛА (МЕНШЕ світла) — без змін
        ...
        // Сценарій 1b: Вимкнули під час планової ТЕМРЯВИ (тобто очікувалось темрява, але фактично було світло довше)
        // Використовуємо інвертовані правила: рахунок diff = фактичне вимкнення - планове вимкнення
        if (lastScheduled.time) {
          const diffInMinutes = differenceInMinutes(latest.time, lastScheduled.time);
          // інтерпретація: diff > 0 => вимкнули пізніше, отже світло було довше; diff < 0 => вимкнули раніше
          if (diffInMinutes >= -30 && diffInMinutes <= 30) {
            scheduleContextMessage = 'ℹ️ Вимкнення відбулося за графіком.';
          } else if (diffInMinutes > 30) {
            // вимкнули значно пізніше — більше світла, позитивна аномалія
            scheduleContextMessage = '💡 Світло було довше, ніж за графіком! Слава Енергетикам!';
            if (diffInMinutes > 120) {
              scheduleContextMessage = '🚨 Ймовірно аномалія/екстрене втручання — світло було значно довше. Слава Енергетикам!';
            }
          } else { // diffInMinutes < -30
            // вимкнули раніше ніж планували — менше світла
            scheduleContextMessage = '🤬 Вимкнули раніше, ніж за графіком. Клята русня!';
          }
        }
      }
      ...
      if (lastScheduled.status === LightStatus.OFF) {
        // Сценарій 2a: Увімкнули під час планової ТЕМРЯВИ (БІЛЬШЕ світла) — без змін
        if (nextScheduled.time && nextScheduled.status === LightStatus.ON) {
          ...
          if (diffInMinutes > 30) {
            ...
            if (diffInMinutes > 120) {
              scheduleContextMessage = '🙏💡 Світло дали більш ніж на 2 години раніше! Слава Богу та Енергетикам!';
            }
          } else if (diffInMinutes < -30) {
        ...
        // Сценарій 2b: Увімкнули під час планового СВІТЛА (тобто в період, коли за графіком мало б бути світло)
        // Тут правильна інтерпретація: якщо фактичне увімкнення пізніше за планове -> світла не було довше за графік (затримка)
        if (lastScheduled.time) {
          const diffInMinutes = differenceInMinutes(latest.time, lastScheduled.time);
          // diff > 0 => фактичне пізніше за планове (затримка, світла не було)
          // diff < 0 => фактичне раніше за планове (повернули раніше)
          if (diffInMinutes >= -30 && diffInMinutes <= 30) {
            scheduleContextMessage = 'ℹ️ Увімкнення відбулося за графіком.';
          } else if (diffInMinutes > 30) {
            // затримка повернення світла
            scheduleContextMessage = '🤬 Світла не було довше, ніж за графіком. Клята русня!';
            if (diffInMinutes > 120) {
              scheduleContextMessage = '🚨 Ймовірно екстрене відключення — повернули значно пізніше. Клята русня!';
            }
          } else { // diffInMinutes < -30
            // увімкнули раніше, ніж планувалось
